...
public class CountryList {
    ...
    public CountryList(final Context context) {
        ...
        try {
            ...
            countryPrefixes = this.context.getResources().getStringArray(R.array.country_prefix_list);
        } catch (Resources.NotFoundException exception) {
            Log.d(TAG, "Country resource arrays are empty.");
            return;
        }
        // Check that resource length is at least matching to avoid NPE.
        if (countryCodes.length == countryNames.length && countryCodes.length == countryPrefixes.length)
            for (int i = 0; i < countryCodes.length; i++)
                this.countries.add(new Country(countryNames[i], countryCodes[i], countryPrefixes[i]));
        else
    ...
    /**
     * The country prefix on a given index.
     * @param position Given position.
     * @return The country code.
     */
    public String getPrefix(final int position) {
        if (this.countries.size() > position)
            return this.countries.get(position).getPrefix();
